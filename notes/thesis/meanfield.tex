...
\end{split}
\end{equation}
Below, we adopt $\sgn_k=1$. (This works for BEC and most part of BCS; and in the final equations, the sign function is taken care of automatically by using $E_{\vk}$.  It is easy to verify that  we arrive at the same final result in the other cases where $\sgn_{k}=-1$. ) Derivatives over $h_{1\vk}$ are\footnote{Here we follow the convention that takes $h_{1\vk}$ and $h_{2\vk}$ as real}
\begin{equation}
\begin{split}
...
\begin{gather}
\frac{2h_{1\vk}}{\sqrt{1-4 h_{1\vk}^2-4 h_{2\vk}^2}} \xi^{ab}_{\vk}+\frac{8 h_{1\vk} h_{2\vk}^2}{\sqrt{1-4 h_{1\vk}^2-4 h_{2\vk}^2} \left(1+\sqrt{1-4 h_{1\vk}^2-4 h_{2\vk}^2}\right)^2}\eta-2U_{\vk\vk'}h_{1\vk'}-2Y_{\vk\vk'}h_{2\vk'}=0
\label{eq:20100909:fullgapa}\\
\frac{2h_{2\vk}}{\sqrt{1-4 h_{1\vk}^2-4 h_{2\vk}^2}} \xi^{ac}_{\vk}-\frac{8 h_{1\vk}^{2} h_{2\vk}}{\sqrt{1-4 h_{1\vk}^2-4 h_{2\vk}^2} \left(1+\sqrt{1-4 h_{1\vk}^2-4 h_{2\vk}^2}\right)^2}\eta-2V_{\vk\vk'}h_{2\vk'}-2Y_{\vk\vk'}h_{1\vk'}=0
...
At the lowest order, the second term in both gap equations, Eq. \ref{eq:20100909:fullgap}, are negligible.  And to the lowest order, the first term in these two equations are equal $D_{1}$ and $D_{2}$ respectively.  
