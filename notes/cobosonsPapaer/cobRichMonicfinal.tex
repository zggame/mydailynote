...
known analytical solution for arbitrary $N$ and interaction strength, so that they were mostly approached through
...
within an "irrelevant" $1/2$ prefactor. This deliberately assigns to each pair an
...
%***Guojun, please put p', I cannot with your bad new commands ...***
...
even if the attracting BCS potential is extremely small, as evidenced below.
...
% the $w_\vk$'s being moreover such that $w_\vk^2=w_\vk$
...
%\gamma^{\dagger}_\vk=-V\,w_\vk\beta^{\dagger}\quad\quad
\beta^{\dagger}=\sum_%
...
Another very interesting feature of the energy $E _N$ of $N$ pairs that
...
In order to have terms in $N(N-1)(N-2)$, we would need topologically connected
interaction processes between 3 pairs. This is why terms in $N(N-1)(N-2)$ and above, cannot exist in the energy of $N$ Cooper pairs, in agreement with Eq.(1) which also reads
...
\end{align}
So that at lowest order in  inverse sample volume, i.e., in $1/\rho_0$, we end with 
%since $% N_\Omega=\rho_0\Omega$ 
...
%with $w_{\mathbf{k} }$ moreover such that $%w_vk^2=1$. 
Note that a separable potential is also required to get
...
The BCS-potential used in the calculation is highly simplified, therefore leaves out certain correction. Nevertherless, it led to the BCS ansatz with the essential feature of superconductivity. And it is of importantce to obtain its solution in canonical ensemble and compare it to the conventional ansatz in grand-canonical ensemble.
...
$(2\times2)$ scatterings only. This explains
why the $N$-pair interaction energy that we have previously found, has terms in $N(N-1)$ but not in $N(N-1)(N-2)
$ and so on...
...
\bibinfo{author}{\bibfnamefont{D.~M.} \bibnamefont{Eagles}},
  \bibinfo{journal}{Physical Review} \textbf{\bibinfo{volume}{186}},
...
\bibinfo{author}{\bibfnamefont{M.}~\bibnamefont{Combescot}} \bibnamefont{and}
  ...
  \bibitem{frenkel}  Wannier excitons have two degrees of freedom while Frenkel excitons have one only. This is why the Frenkel exciton many-body physics has similarities with the one of Cooper pairs (see  W. Pogosov, M. Combescot, Euro. Phys. J. B, Vol. 68, 161(2009), Vol 68, 183 (2009)).
