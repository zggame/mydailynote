...
problem
%This problem has not been solved yet in a fully controllable way, even for the"reduced" BCS potential in which only up and down spin electrons with opposite momenta are coupled.
...
% Although somewhat different, this procedure has the same goal as the one proposed by Leggett [Leggett], where the overlap is changed by changing the strength of the interaction between pairs. In our procedure, the crossover only occurs among pairs which are inside the potential layer since we keep a constant "frozen" core. However, by contrast with previous variational BCS-like approaches [Eagles, Leggett], we uses the exactsolution to the many-pair Schr\"{o}dinger equation provided by Richardson. This a priori allows us to securely tackle the most interesting domain of the cross-over, which is when the pairs turn from the dilute and the dense regimes. The interest to his domain is due to the fact that, as it was realized long time ago, the BCS ansatz for the wave function can collapse in this region [Leggett]. Notice that the crossover problem within Richardson equations was studied inRef. [Dukelsky] by using an assumption that ground-state $R$'s form a single arc in the c
 omplex plane and by using a continous approximation.

%Although Richardson procedure drastically simplifies quantum many-body problem, the resolution of Richardson equations is a quite complicated problem.
...
Another very interesting feature of the energy $E _N$ of $N$ pairs,
...
mixed with fermion exchanges between pairs, ends by producing effective scatterings which are (2x2) only. Indeed,
in order to have terms in $N(N-1)(N-2)$, we need topologically connected
interaction processes between 3 objects. This is why terms in $N(N-1)(N-2)$ and above, cannot exist in the energy of $N$ Cooper pairs, in agreement with Eq.(1) which also reads
...
^{\prime}}w_{\mathbf{k} }$.
%with $w_{\mathbf{k} }$ moreover such that $%w_\vk^2=1$. 
...
\bibinfo{author}{\bibfnamefont{R.~W.} \bibnamefont{Richardson}},
  \bibinfo{journal}{????????????} \textbf{\bibinfo{volume}{3}},
  \bibinfo{pages}{???} (\bibinfo{year}{1965}).
...
\bibitem[{\citenamefont{Pogosov et~al.}()\citenamefont{Pogosov, Combescot, and
  Crouzeix}}]{combescotBCS}
