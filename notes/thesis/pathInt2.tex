...
Multiply both side with $(1+TG)$,  where $T$ is the scattering matrix for open-channel, and $G=(\omega-H_{0})^{-1}$ is the Green's function for a free pair in the open-channel
...
\parbox{0.8\textwidth}{\small  Here $E_{0}=a_{\text{bg}}\mathcal{K}k_{F}$ }
\end{center}
\end{figure}

If only taking this effect into consideration, the gap equation and the (open-channel) number equation are simplified to 
\begin{gather}
1=-\mbr{\frac{4\pi{\tilde{a}_{s}(\mu,\lambda_{1})}}{m}\sum(\nth{2E_{\vk}}-\nth{2\epsilon_{\vk}})}\\
N=\sum_\vk\frac{E_\vk-\xi_\vk}{2E_\vk}\\
...
\end{gather}
In the last equation, we assume $a_{\text{bg}}\ll{}a_{0}$ and we only study situation close to resonance, where $\mathcal{K}\gg{}\delta \,\text{or}\,\mu$.  From the two-body analysis of the Feshbach resonance, we know the numerator in the Eq. \ref{eq:pathInt2:simplenarrowAs} is closely related to the characteristic scale $\delta_{c}$ in Eq. \ref{eq:intro:deltaC}
...
\end{equation}
In Fig. \ref{fig:pathInt2:simpleNarrowMuDt}, $\delta_{c}={E_{0}^{2}}/{E_{F}}$, we see the two lines correspond to $\delta_{c}=10E_{F}$ and $100E_{F}$.
