...
\item[$\delta_{c}$] Energy scale of the  detuning from resonance where closed-channel takes substantial weight. (Page \pageref{eq:intro:deltaC})
\item[$a_{0}$] Average inter-particle distance, $a_{0}k_{F}\sim1$.
\item[$r_{c}$] Potential range.  All  potential are zero outside $r_{c}$.
\item[$a_{c}$] Characteristic size of bound state in resonance ($\phi_{0}$) if close-channel is isolated. It is proportional  to inverse of $\kappa$.
...
\item[$E_{\vk}$] $E_{\vk}$ corresponds the elementary fermionic excitation energy for $\vk$ in single channel.  $E_{\vk}=\sqrt{\epsilon_{\vk}^{2}+\Delta^{2}}$, where $\epsilon_{\vk}=\hbar^{2}k^{2}/2m$ is the kinetic energy. In two-channel model, it is defined similarly, $E_{\vk}=\sqrt{\epsilon_{\vk}^{2}+\Delta_{1}^{2}}$.  It is the zeroth order of  energy for two elementary fermionic excitation modes. (Page \pageref{eq:pathInt:G0})
