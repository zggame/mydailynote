...
Alternatively, we can make a rough estimation using two-fermion molecule gas. We assume the molecule size is $a_{c}$ and the total number is $N$.  If assuming further that the bound-state is close-to-threshold,   wave function can then be written as $A/(k^{2}+\kappa^{2})$, where $\hbar^{2}\kappa^{2}/2m=E_{b}$, (see Appendix \ref{sec:pathInt2:short-range}). $A$ can be obtained by normalization, $\sum_{k=0}^{1/r_{c}}\abs{\psi}^{2}\sim{}N$. Now  we consider only particles within range $E_{F}\ll\kappa$, i.e., the typical many-body scale of open-channel occupation.  The total particle in this range is roughly $N\cdot(k_{F}a_{c})$, which is much smaller than $N$. Put this into the perspective of two-channel problem, the low-momentum ($k\lesssim{}k_F$), is still dominated by open-channel component even when the total number of closed-channel is comparable or higher than the total number  of open-channel because closed-channel is mostly in high-momentum state.     

We  introduce several concepts and theories important in the following sections. A detailed  study about the narrow resonance problems in two channels is then presented in Chapter \ref{ch:path2}. An alternative approach using ansatz in mean-field level is included as Appendix \ref{ch:mean}.  Chapter \ref{ch:conclusion} discusses and concludes our approach .  
...
\end{equation}
where $c_{\vk}=\alpha( v_{\vk}/u_{\vk})$.
Moving toward BEC side, the inter-particle attraction becomes stronger;  so the ``wave function'' shrinks.  At BEC side, two-fermion molecules are smaller than inter-particle distance and therefore well-defined. However, the binding energy now is much higher than typical many-body energy scale and condensation does not happens at the same time when molecules form.  Nevertheless, we can still consider the condensation at low enough temperature and therefore with the same framework as BCS.  
