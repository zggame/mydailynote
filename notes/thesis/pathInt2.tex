...
\end{equation}
\end{unsure}
In the narrow resonance, the weight of the closed-channel becomes substantial quickly.  We can calculate the weight of the closed-channel according to  Eq. \ref{eq:pathInt2:alpha}
\begin{equation}\tag{\ref{eq:pathInt2:alpha}}
\alpha=\frac{\sum_{\vk\vk'}{\phi_{\vk}^{*}}{Y_{\vk\vk'}}{h_{1\vk'}}}{\br{-E_{b}+\eta-2\mu-\lambda_{1}}}
\end{equation}
  $Y$, as a short-range interaction, in the above integral only picks up the short-range part of $h_{1}(r)$. The short-range part of $h_{1}$ is assumed to be proportional to the short-range of two-body wave function. Its normalization, however, comes from many-body physics. If we assume the zeroth order form of $h_{1}=\Delta_{1}/2E_{\vk}$,   Zhang has derived the form of  $h_{1}(\vr)$ in the real space, (Eq. (25) in \cite{shizhongSumRule})
  \begin{equation}
...
\end{equation}
From Eq. \ref{eq:intro:deltaC} above, we can replace $\mathcal{K}a_{bg}$ with $\delta_{c}$, and we divide both side with the open-channel density $n_{o}=\br{\frac{mE_{F}^{(o)}}{\hbar^{2}}}^{2/3}\frac{\sqrt{2}}{3\pi^{2}}$, we find
\begin{equation}
\beta=\frac{n_{c}}{n_{o}}\approx\frac{3\sqrt2}{128}\frac{\Delta_{1}^{2}}{{E_{F}^{(o)}}^{3/2}\delta_{c}^{1/2}}
\end{equation}
Notice that here $E_{F}^{(o)}$ is the Fermi energy of the open-channel and might be substantially smaller than the total Fermi energy, $E_{F}^{(tot)}$ when moving toward the BEC end.   This form is consistent with the formula obtained by Zhang previously \cite{shizhongUniv}.  $\beta$ is proportional to $\Delta_{1}^{2}$.  At the BCS limit, $n_{o}\approx{}n_{tot}$ and $E_{F}^{(o)}$ equals the Fermi energy calculated by the total density.  $\Delta_{1}$ is exponentially small, there is only very small amount of atoms in the closed-channel.  Moving toward the BEC side, $\Delta_{1}$ increases and the closed-channel density increases as well.  Because the maximum of the $n_{c}$ is the total density $n_{tot}$, we can see that $\Delta_{1}$ saturates.  The narrower the resonance, the sooner it saturates.  In fact, at the universality point, $\Delta_{1}$ is in the order of the Fermi energy, and $\beta$ is already substantial if the resonance is very narrow, i.e. $\delta_{c}$ is much smaller than $E_{F}$.  

We can see an example of the narrow resonance ($\delta_{c}=0.001E_{F}^{(tot)}$) in the Fig. \ref{fig:pathInt2:narrow}. $\Delta_{1}$ saturates at about $0.8E_{F}$. The relative weight of the open channel is defined as 
\begin{equation}
...
\caption{The chemical potential $\mu$, gap $\Delta_{1}$ and open-channel relative weight $\beta_{o}$ over crossover with the Feshbach resonance} 
\label{fig:pathInt2:narrow}
\parbox{0.8\textwidth}{\small Note that here the detuning $\delta$, gap in the open-channel $\Delta_{1}$ and chemical potential $\mu$ are rescaled with the the Fermi energy of the total density, $E_{F}^{(tot)}$.  We have taken $\delta_{c}=0.001E_{F}^{(tot)}$. We used the detuning from the resonant point  instead of $1/a_{s}k_{F}$ in the single-channel because the additional shift considering in Eq. \ref{eq:pathInt2:simplenarrowAs}.}
\end{center}
\end{figure}
...
\end{equation}
Here the coefficient $\Delta_{1,\text{sat}}^{(0)}$ is common for one specific Feshbach resonance.  It is possible to observe this saturation from experimentally by measuring the fermionic excitation spectrum.  
