...
The two-channel Hamiltonian can be written as ($2\times2$ matrix for  (open,closed) ``spinor'')
...
\end{pmatrix}
\end{equation}
where  zero of energy is counted from that of two open-channel atoms with infinite separation. $\eta$ is the absolute difference in Zeeman energy of two channels.  All the interactions are short-range.  For a s-wave solution we have 
\begin{equation}
...
\end{equation}
We denote $\phi_{0}$ as the wave function  in resonance and $c_{0}$ is its coeeficient.  Here we assume the energy difference between eigenstates are larger than other energy scales in the problem; hence $c_{0}$ dominates any other $c_{i}$'s.  $\chi_{c}\approx{}c_{0}\phi_{0}$.  Na\"{i}vely speaking, the resonance happens at the point where the closed-channel bound state level is exactly at the threshold of the open-channel.  Therefore we introduce the relative detuning, $\tilde\delta=\eta-E_{b}$. Comparing Eq. \ref{eq:intro:open} and Eq. \ref{eq:intro:sch2}, it is not hard to find
