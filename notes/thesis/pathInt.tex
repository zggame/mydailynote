...
\end{equation}
Here this matrix is in the Nambu Spinor space.  At the  mean field level, we seek a tempo-spacial homogeneous solution of $\Delta_0$, i.e., $\Delta_{0}$ is a $\delta$-function in the frequency-momentum space, $\Delta_{0,\omega_{n},\vk}=\Delta_{0}\delta(\omega_{n}=0)\delta(\vk=0)$. We can find the Nambu Green function from Eq. (\ref{eq:pathInt:nG}) in momentum space
...
\end{equation}
Here $p$ is the four momentum, $p=(\omega_{n},\vp)$, and $\omega_n$ is the Matsubara frequency of Fermions.  $\xi_{\vk}=\epsilon_{\vk}-\mu$, $\epsilon_{\vk}=\vk^{2}/2m$,  $E_\vp=\sqrt{\xi_\vp^2+\abs{\Delta_0}^2}$.  And the saddle point equation can be rewritten as 
\begin{equation}
