\documentclass[aps,prb,superscriptaddress,showpacs,preprint,lengthcheck]{revtex4-1}
...
Pauli exclusion principle between a fixed number of interacting fermions is however  known  to be quite difficult, especially when these fermions are paired. 
...
and also by Leggett \cite{LeggettCrossover}, through the change of the interaction strength between pairs. In their approach, the number of pairs is fixed, so that Pauli blocking does not change when the overlap changes while it changes when the pair number increases. Consequently, the two types of overlap change do not involve the same physics. This is why to change the overlap by changing the pair number at constant potential is a fully relevant problem, complementary to the one studied in the past. 
*****************Ortiz and Dukelsky applied Richardson approcah to crossover problem with a different prerspective, where overlap varies due to interaction strength\cite{crossoverRich}.  Some interesting comparison between the BCS ansatz and the Richardson wave-function is carried.  
...
This allowed us to demonstrate that, in the dilute limit on the single Cooper pair scale, i.e., for $N$ arbitrary large but $N/N_{c}$ small, 
...
layer, taken as constant. $N_{c }=\rho _{0}\epsilon _{c} $ while $N_{\Omega }=\rho _{0}\Omega $ is the number of free pair states in this
...
the exact energy of $N$ pairs in the normal state whatever this pair number. Indeed, for a constant density of states $\rho _{0}$, the kinetic energy of $N$ free pairs just above the frozen Fermi sea, is given by
\begin{multline}
{E}_{N}^{\text{(}normal)}= \\
...
between electron pairs. This leads us to conclude that 
the Richardson's parameters $R_i$  have $N$ different values just because 
of Pauli blocking between Cooper pair components. As a direct consequence, this makes the $N$-pair ground state fundamentally different from the BCS antsatz.


In section IV
%\ref{sec:conn}
, we briefly question this BCS wave function ansatz for condensed pairs compared to the exact Richardson's form, in the light of our general understanding of the many-body physics of composite bosons.
...
\beta^{\dagger}\left|F_0\right> 
\end{eqnarray}
We then sum $\left(\mathbf{k} _1,\mathbf{k} _2,\mathbf{k} _3\right) $. By calculating the sums of the two brackets as for two pairs, Eqs. (\ref{eq:1-v}) and (\ref{eq:deltakk}) then yield
...
An important feature of the energy $E _N$ for $N$ pairs that
...
diagrams between 3 pairs. The diagram of Fig.\ref{fig:threeP} evidences the fact that one out of the three pairs do not participate to the scattering, so that the three pairs are not connected. In the case of $N$ pairs, this is $N-2$ out of the $N$ pairs. which are not connected. So that connections seem to exist between two pairs only. 

This however is not enough to explain that higher order terms do not exist in the energy because Wannier excitons have terms in $N(N-1)(N-2)$ and so on\cite{monicOdil}  while the hamiltonian acting on $N$ Wannier excitons seems to leave $N-2$ excitons unchanged. Indeed, let $B_i^\dagger$ be the create operator of the $i$ exciton with energy $E_i$, i.e., $(H-E_i)B^\dagger_i\left|0\right>=0$.  We do have, as in Eq. (\ref{eq:SchThree}), 

\begin{equation}\label{eq:HB3}
\begin{split}
&HB^{\dagger}_{i_1}B^{\dagger}_{i_2}B^{\dagger}_{i_3}\left|F_0\right>=\\
...
&\left.+B^{\dagger}_{i_1}B^{\dagger}_{i_2}\left[H,B^{\dagger}_{i_3}\right]  \right\}\left|F_0\right> 
\end{split}
\end{equation} 
We not introduce the $i$ exciton create operator $V^\dagger_i$ defined as
\begin{equation}
\left[H,B^{\dagger}_{i}\right]=E_iB^\dagger_i+V^\dagger_i
\end{equation}
 which is similarly with Eqs. (\ref{eq:betaH},\ref{eq:vbeta}). This operator is such that $V^+_i\left|0\right>=0$, as readily seen from the above equation acting on a vacuum.  From it, we construct the interaction scatterings of two execitons through
 \begin{equation}  
\left[V^{\dagger}_{i},B^{\dagger}_{j}\right] 
=\sum\xi\left(\begin{smallmatrix}n&j\\m&i\end{smallmatrix}%
\right)  B^{\dagger}_{m}B^{\dagger}_{n}
\end{equation}
which is similar to Eq. (\ref{eq:vBeta}). When used into Eq. (\ref{eq:HB3}), this leads to
\begin{equation}\label{eq:HEEE}
\begin{split}
&(H-E_{i_1}-E_{i_2}-E_{i_3})B^{\dagger}_{i_1}B^{\dagger}_{i_2}B^{\dagger}_{i_3}\left|F_0\right>\\
=&\sum{}B^\dagger_mB^\dagger_n\left\{
	\xi\left(\begin{smallmatrix}n&i_2\\m&i_1\end{smallmatrix}\right)B^{\dagger}_{i_3}\right.\\
	&\left.	+\xi\left(\begin{smallmatrix}n&i_3\\m&i_2\end{smallmatrix}\right)B^{\dagger}_{i_1}	+\xi\left(\begin{smallmatrix}n&i_1\\m&i_3\end{smallmatrix}\right)B^{\dagger}_{i_2}\right\}
\end{split}
\end{equation}
As for Eq. (\ref{eq:vThree2}), one out of the three excitons seems unaffected in the scattering process.

Here comes the crucial difference between Wannier exciton and Cooper pairs. Wannier exciton, being made of $a^\dagger_{\vk_1}b^\dagger_{\vk_2}$ pairs, have two degrees of freedom. As a direct consequence, the two electrons and the two holes of two excitons can be associated in two different ways to form two excitons. It is possible to show that (\cite{CobosonPhysicsReports})
...
\end{split}
\end{equation}
We can then note that the fact that the $B^\dagger_{i_3}$ exciton does not participate to the scattering is rather artificial because, using Eq. (\ref{eq:BB}), we can rewrite the first term in the RHS of Eq. (\ref{eq:HB3}) as
...
\end{equation}
This shows that the $(i_1,i_2,i_3)$ excitons are involved in the $3\times3$ connected diagram shown in 
\begin{figure}[htb]
...
If we now consider $\beta^\dagger_\vk$ pairs, the $\vk$ electron with up spin can be associated to the $-\vk$ electron with down spin.  So that as equation similar to Eq. (\ref{eq:BB}) do not exist.  As a result the $\vk_3$ pair in Eq. (\ref{eq:vThree2}) cannot be mixed with the $\vk'_2$ pair as in Eq. (\ref{eq:3BLambdaXi}) to generate a $3\times3$ connected diagram. 
...
BCS ansatz for the condensed pair wave function since this ansatz reduces to $\left(B^{\dagger}\right)
^N\left|F_0\right> $ when projected into the $N$-pair subspace. In this ansatz,  \emph{all} the pairs are taken as condensed into the same state. This is hard to accept for composite bosons due to Pauli blocking between pairs which made each added pair necessarily different from the previous ones.
...
energies parameters read for two pairs read as $R_1=R+iR^{\prime}$ and $R_2=R-i{}R^{\prime}$ with $R$
...
^2$ as in the BCS ansatz, we must neglect terms in $R'^2$, i.e., in $1/\rho_0$. However, these $1/\rho_0$ terms are precisely those which make $E _1$ different from $E _2/2\approx1+1/\rho_0+\epsilon_c/\rho_0\Omega$; so that the replacement of $B^{\dagger}(R_1)B^{%
\dagger}(R_2)$ by a ``condensed two-pair state'' $\left(B^{\dagger}(E _2/2)\right) ^2$ with $E_2$ \emph{different }from $E_1$ is fully inconsistent because we then keep in this two-pair
...
limit when $N$ is very large. It however is possible to show that, for $N$ large but still in the dilute limit on the single pair scale, the $R_i$'s stay two by two complex conjugate, the imaginary part of the $(R_i - R_j)$ differences getting larger and larger as $\sqrt{N\epsilon_c/\rho_0}$ when $N$ increases. By using a similar procedure as the one we used for $N=2$, we hardly see how, 
...
creation operator for all  pairs when $N$ ventures outside the dilute limit since nothing really changes in the behavior of the $R_i$'s.
...
from the exact one, while giving correct energy. Direct
...
ansatz in the thermodynamical limit, seems to us necessary to more deeply understand basic superconductivity. The coboson many-body formalism we have here constructed should appear as quite valuable because it gives a fresh view to this quite old and famous field, its Shiva diagram representation helping to support physical understanding.
*****************************************
Recently, Ortiz and Dukelsky\cite{crossoverRich} shows that at thermal dynamic limit that Richardson approach gives the same energy, however, the Richardson wave function is substantially different from BCS ansatz in many way.   
