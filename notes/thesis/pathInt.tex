...
We try to solve this system by introduce Hubbard-Stratonovich transformation.   Introduce an auxiliary field $\Delta(\vr,\tau)$ coupled with Cooper channel $\psi_{\uparrow}(\vr,\tau)\psi_{\downarrow}(\vr,\tau)$. %Here we follow the normal notation from path integral, $r$ is four tempo-space coordinator.  
