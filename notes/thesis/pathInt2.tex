...
\caption{The chemical potential $\mu$, the gap $\Delta_{1}$ and the open-channel relative weight $\beta_{o}$ over crossover with the narrow Feshbach resonance} 
\label{fig:pathInt2:narrow}
\parbox{0.8\textwidth}{\small  The gap in the open-channel $\Delta_{1}$ and the chemical potential $\mu$ are rescaled with the the Fermi energy of the total density, $E_{F}^{(tot)}$.  While the detuning $\delta$ is rescaled with $E_{0}$ (see Eq. \ref{eq:pathInt2:E0}). We have taken $\delta_{c}=0.001E_{F}^{(tot)}$. We used the detuning from the resonant point  instead of $1/a_{s}k_{F}$ in the single-channel because the additional shift considering in Eq. \ref{eq:pathInt2:simplenarrowAs}.}
\end{center}
\end{figure}
