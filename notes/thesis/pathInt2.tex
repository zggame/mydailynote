...
In the broad-resonance, where only the BCS pairing in open channel needs to be considered, $T$ is enough to diagonalize $G^{-1}$ and $L$ is simply an identity matrix.  %Here we will try to approximate it to the first order correction due to Pauli exclusion between two-channel.  
...
We study two phase fluctuation modes in the next two sections. 
\subsection{Two-channel in-phase phase fluctuation }
...
\subsection{Out-of-phase phase fluctuation}
...
=&\mtrx{\av{\bar{\psi}_{b,\vk}\bar{\psi}_{a,-\vk}}e^{-i\theta}&\av{\bar{\psi}_{c,\vk}\bar{\psi}_{a,-\vk}}e^{+i\theta}}\tilde{U}^{}\tilde{U}^{-1}\tilde{U}
\mtrx{\av{\psi_{b,\vk}\psi_{a,-\vk}}e^{+i\theta}\\\av{\psi_{c,\vk}\psi_{a,-\vk}}e^{-i\theta}}\\
...
=&\mtrx{h_{1,\vk}^{*}&h_{2,\vk}^{*}}\mtrx{U&Ye^{-i2\theta}\\Y^{*}e^{+i2\theta}&V}\mtrx{h_{1,\vk}\\h_{2,\vk}}\\
=&\Delta^{(0)}{}^{\dg}\tilde{U}^{-1}\Delta^{(0)}
+\mbr{Y(e^{-i2\theta}-1)h_{1,\vk}^{*}h_{2,\vk} +Y^{*}(e^{+i2\theta}-1)h_{1,\vk}h_{2,\vk}^{*}}
\end{align*}
Note that here we take the mean-field expectation of pairs of fermions $h_{1,\vk}=\av{\psi_{b,\vk}\psi_{a,-\vk}}$, $h_{2,\vk}=\av{\psi_{c,\vk}\psi_{a,-\vk}}$, and their complex conjugates\footnote{$\psi\psi$ and $\bar\psi\bar\psi$ are complex conjugate in the mean field.}.  Comparing with the action of the in-phase phase fluctuation (Eq. \ref{eq:pathInt2:actionPhase}), we find one more terms in the new action.  
...
S_3[\theta]=&\int{dx}\mbr{Y(e^{-i2\theta}-1)\av{\bar{\psi}_{b}\bar{\psi}_{a}}\av{\psi_{c}\psi_{a}}
     +Y^{*}(e^{+i2\theta}-1)\av{\bar{\psi}_{c}\bar{\psi}_{a}}\av{\psi_{b}\psi_{a}}}
\end{align}
\end{subequations}
Let us look at $S_{3}$ first.  The two terms in the parenthesis are complex conjugate and 
\begin{subappendices}
