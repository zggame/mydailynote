...
the only correction in the next order.  Comparing to first term, it is in the order of $D_2^2/(\xi_\vk+\eta)\xi_vk$.  This quantities can add up to a substantial quantity as seen in close-channel number equation Eq. \ref{eq:pathInt2:closeD2} when summed over all momentum to scale of $\eta$.  However, the main part $D_1^2/E_\vk^3$ skews in low-momentum heavily, which makes sure the second term only contribute perturbatively. 
